...
public class DynamicGridView extends GridView {
    ...
    public void init(Context context) {
        ...
        try {
            shouldShowSelectorMethod = AbsListView.class
                    .getDeclaredMethod("shouldShowSelector");
            shouldShowSelectorMethod.setAccessible(true);
    ...
    private View downItemView = null;
    @Override
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        boolean shouldShowSelector = false;

        try {
            shouldShowSelector = (Boolean) shouldShowSelectorMethod
                    .invoke(this);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        boolean rlt;
        if (child == downItemView && shouldShowSelector) {
            int l = child.getLeft();
            int t = child.getTop();
            int w = child.getWidth();
            int h = child.getHeight();

            canvas.scale(1.1f, 1.1f, l + w / 2, t + h / 2);
            rlt = super.drawChild(canvas, child, drawingTime);
        } else {
            rlt = super.drawChild(canvas, child, drawingTime);
        }
